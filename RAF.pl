...
open (SAMPLES, "/local/projects-t3/SerreDLab-3/kieran.tebben/mali/sample_lists/pfpo_IDs.txt");
my @sample_names;
...
#	$sample =~ s/\.mpileup*//;
	...
	open(OUT, ">", "/local/projects-t3/SerreDLab-3/kieran.tebben/mali/$out");
	print OUT "chromosome\tposition\tcoverage\treads_with_ref_allele\tRAF\n";
	
	chomp $sample;
	my $mpileup_path = "/local/projects-t3/SerreDLab-3/kieran.tebben/mali/${sample}.subset.rmdup_subset.mpileup";
